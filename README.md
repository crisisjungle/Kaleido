...
# 可选：deepseek-v4-pro
# 可选：enabled / disabled / auto
DEEPSEEK_THINKING_MODE=disabled
DEEPSEEK_REASONING_EFFORT=high
ZEP_API_KEY=your_zep_api_key
```

兼容说明：旧的 `deepseek-chat` / `deepseek-reasoner` 在代码里会自动映射到 DeepSeek V4，对应关系分别是 `deepseek-v4-flash` 非思考 / 思考模式；但官方文档说明这两个旧模型名会在 `2026-07-24` 停用，建议后续显式改成 V4 名称。
