...
- immutable revision metadata: artifact ID, revision, source hash, content hash, and previous artifact reference.
...
`foundation.step1.v3` carries the Step 1 reference into Step 2. `scenario_planning.v2` consumes normalized event and policy records directly. Keyword planning remains compatibility behavior only when no semantic keys are available.
...
Location text, event or baseline text, variable text and explicit variable structures, context, subjects, goals, revision instructions, uploaded documents, and map context enter the semantic layer. The background report consumes the resulting artifact and does not reinterpret the raw request.

Scene seeds, projects, and simulations retain a semantic artifact reference. Browser storage is only a navigation recovery cache. Active variables preserve type, direction, intensity, expected effects, targets, time, controlled keys, and source through the Step 1 to Step 2 handoff.
...
Editable event and policy cards are projected from the semantic artifact. `/api/simulation/prepare` normalizes the complete edited input and writes a new revision before building `scenario_planning.v2`.
...
- The Wuhan frozen planning fixture uses `scenario_planning.v2`; its broader risk/runtime compatibility contract remains independently versioned.
