...
logger = get_logger("envfish.graph_builder")
...
class GraphBuilderService:
    ...
    def _call_zep_with_retry(
        ...
        """Retry transient Zep SDK failures with exponential backoff."""
        last_exception = None
        delay = initial_delay
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    logger.warning(
                        f"Zep {operation_name} attempt {attempt + 1} failed: {str(exc)[:160]}, "
                        f"retrying in {delay:.1f}s..."
                    )
                    time.sleep(delay)
                    delay *= 2
                else:
                    logger.error(f"Zep {operation_name} failed after {max_retries} attempts: {str(exc)}")
    ...
    def build_graph_async(
        ...
        TaskExecutor(self.task_manager).start(
            task_id=task_id,
            target=lambda: self._build_graph_worker(
    ...
    def create_graph(self, name: str) -> str:
        """创建Zep图谱（公开方法）"""
        graph_id = f"envfish_{uuid.uuid4().hex[:16]}"
        ...
        for i in range(0, total_chunks, batch_size):
            ...
            try:
                batch_result = self._call_zep_with_retry(
                    ...
                    f"add batch {batch_num}/{total_batches} to graph {graph_id}"
                )
    ...
    def delete_graph(self, graph_id: str):
        """删除图谱"""
        self._call_zep_with_retry(
            lambda: self.client.graph.delete(graph_id=graph_id),
