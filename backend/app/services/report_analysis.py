...
class ReportAnalysisService:
    ...
    def __init__(self, report_id: str):
        ...
        self.mechanism_artifacts = self._load_mechanism_artifacts()
    ...
    def _load_mechanism_artifacts(self) -> Dict[str, Any]:
        ...
        mechanism_graph = self._read_simulation_json("mechanism_graph.json", {}) or {}
        agent_blueprints = self._read_simulation_json("agent_blueprints.json", []) or []
        validated_relation_graph = self._read_simulation_json("validated_relation_graph.json", {}) or {}
        simulation_audit = self._read_simulation_json("simulation_audit.json", {}) or {}
        relation_ledger = self._read_simulation_jsonl("relation_discovery_ledger.jsonl", []) or []
        round_reasoning = self._read_simulation_jsonl("round_reasoning_ledger.jsonl", []) or []
        return {
            "scenario_model": scenario_model if isinstance(scenario_model, dict) else {},
            "mechanism_graph": mechanism_graph if isinstance(mechanism_graph, dict) else {},
            "agent_blueprints": agent_blueprints if isinstance(agent_blueprints, list) else [],
            "validated_relation_graph": validated_relation_graph if isinstance(validated_relation_graph, dict) else {},
            "simulation_audit": simulation_audit if isinstance(simulation_audit, dict) else {},
            "relation_ledger": relation_ledger if isinstance(relation_ledger, list) else [],
            "round_reasoning": round_reasoning if isinstance(round_reasoning, list) else [],
        }
        ...
        for edge in agent_relationships:
            ...
            add_edge({
                ...
                "attributes": {
                    ...
                    "relation_label": edge.get("relation_label") or relation_type,
                    "mechanism": edge.get("mechanism") or edge.get("rationale") or "",
                    "trigger_conditions": edge.get("trigger_conditions") or [],
                    "latency": edge.get("latency") or "",
                    ...
                    "edge_origin": edge.get("origin") or "seed_profile",
                    "validation_status": edge.get("validation_status") or "",
    ...
    def _build_mechanisms_tab(self) -> Dict[str, Any]:
        ...
        accepted_relations = [
            item for item in relation_ledger
            if isinstance(item, dict) and item.get("status") == "accepted"
        ]
        latest_reasoning = {}
        if round_reasoning:
            latest_reasoning = max(
                [item for item in round_reasoning if isinstance(item, dict)],
                key=lambda item: int(item.get("round") or 0),
                default={},
            )
        return {
            "tab": "mechanisms",
            "available": bool(scenario_model or mechanism_graph or round_reasoning),
            ...
            "counts": {
                ...
                "validated_relations": validated_relation_graph.get("edge_count") or len(accepted_relations),
                "reasoning_rounds": len(round_reasoning),
                "fallback_relation_count": audit.get("fallback_relation_count", 0),
            },
        }
    ...
    def get_overview(self) -> Dict[str, Any]:
        ...
        payload = {
            ...
            "node_stats": {
                ...
                "mechanism_node_count": len((self.mechanism_artifacts.get("mechanism_graph") or {}).get("nodes") or []),
                "mechanism_edge_count": len((self.mechanism_artifacts.get("mechanism_graph") or {}).get("edges") or []),
                "reasoning_round_count": len(self.mechanism_artifacts.get("round_reasoning") or []),
    ...
    def _extract_related_mechanisms(self, subgraph: Dict[str, Any]) -> List[Dict[str, Any]]:
        mechanism_edges = {
            str(item.get("id") or ""): item
            for item in (self.mechanism_artifacts.get("mechanism_graph") or {}).get("edges") or []
        ...
        for edge in subgraph.get("edges") or []:
            ...
            ids = attrs.get("mechanism_edge_ids") or []
            if isinstance(ids, str):
                ids = [ids]
            for mechanism_id in ids:
                mechanism_id = str(mechanism_id or "")
                if not mechanism_id or mechanism_id in seen:
                    continue
                mechanism = mechanism_edges.get(mechanism_id)
                if not mechanism:
                    continue
                seen.add(mechanism_id)
                related.append(mechanism)
        return related[:10]
    ...
    def get_node_context(self, node_id: str, round_range: Any = None) -> Dict[str, Any]:
        ...
        related_mechanisms = self._extract_related_mechanisms(subgraph)
    ...
    def _build_deterministic_sections(self, context: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        related_mechanisms = context.get("related_mechanisms") or []
        ...
        for mechanism in related_mechanisms[:4]:
            relation_items.append({
                "label": mechanism.get("relation_label") or "机制链",
                "content": mechanism.get("mechanism") or mechanism.get("description") or "",
                "source_type": "机制工件",
            })
    ...
    def _generate_llm_sections(self, context: Dict[str, Any], deterministic_sections: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        compact_context = {
            ...
            "related_mechanisms": (context.get("related_mechanisms") or [])[:6],
            "subgraph": {
