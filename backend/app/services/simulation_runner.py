...
logger = get_logger('envfish.simulation_runner')
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        engine_mode = config.get("engine_mode", "oasis")

        # 初始化运行状态
        time_config = config.get("time_config", {})
        minutes_per_round = max(int(time_config.get("minutes_per_round", 30)), 1)
        if engine_mode == "envfish":
            total_rounds = int(time_config.get("total_rounds", 8))
    ...
    def stop_simulation(cls, simulation_id: str) -> SimulationRunState:
        ...
        if state.runner_status not in [RunnerStatus.RUNNING, RunnerStatus.PAUSED, RunnerStatus.COMPLETED]:
            if not (process and process.poll() is None) and not env_alive:
    ...
    @classmethod
    def _read_jsonl_file(cls, file_path: str, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        if not os.path.exists(file_path):
            return []
    ...
    @classmethod
    def get_envfish_artifacts(cls, simulation_id: str) -> Dict[str, Any]:
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        try:
            from .simulation_manager import SimulationManager
            resolved_dir = SimulationManager().resolve_artifact_dir(simulation_id, create_if_missing=False)
            if resolved_dir:
        ...
        if os.path.exists(latest_snapshot_path):
            try:
                with open(latest_snapshot_path, "r", encoding="utf-8") as handle:
                    artifacts["latest_snapshot"] = json.load(handle)
            except Exception:
                pass
        ...
        artifacts["round_snapshots"] = cls._read_jsonl_file(
            os.path.join(sim_dir, "round_state_matrix.jsonl"), limit=96
        )
        ...
        artifacts["interviews"] = cls._read_jsonl_file(
            os.path.join(sim_dir, "interviews.jsonl"), limit=64
        )
            ...
            artifacts["regional_scores"] = [
                {
                    ...
                    "uncertainty_band": item.get("uncertainty_band"),
                    **(item.get("state_vector") or {}),
                }
                for item in regions
            ]
        else:
    ...
    def inject_variable(
        ...
        ipc_client = SimulationIPCClient(sim_dir)
        if not ipc_client.check_env_alive():
            raise ValueError(f"模拟环境未运行或已关闭，无法注入变量: {simulation_id}")

        response = ipc_client.send_inject_variable(variable=variable, timeout=timeout)
        if response.status.value == "completed":
            return {
                "success": True,
                "result": response.result,
    ...
    @classmethod
    def _get_interview_history_from_file(
        cls,
        file_path: str,
        agent_id: Optional[int] = None,
        limit: int = 100
    ...
    def get_interview_history(
        ...
        if not results:
            results.extend(
                cls._get_interview_history_from_file(
                    os.path.join(sim_dir, "interviews.jsonl"),
                    agent_id=agent_id,
                    limit=limit,
                )
            )
