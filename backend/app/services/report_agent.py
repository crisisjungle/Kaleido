...
logger = get_logger('envfish.report_agent')
...
【EnvFish工件总览】
读取EnvFish模拟工件，汇总场景模式、扩散模板、区域状态、干预记录与反馈链。

【使用场景】
- 需要快速了解当前EnvFish模拟的整体结构
- 需要确认是否存在区域级扩散、干预摩擦和次生反馈
- 需要为报告规划章节提供工件级上下文
...
TOOL_DESC_ENVFISH_SPREAD = """\
【EnvFish区域扩散摘要】
读取EnvFish工件中的区域状态和扩散事件，输出区域级传播态势。
...
TOOL_DESC_ENVFISH_VULNERABILITY = """\
【EnvFish脆弱性排序】
读取EnvFish工件中的状态矩阵，输出区域与关键角色的脆弱性排序。

【返回内容】
- 脆弱性排名
- 暴露、扩散压力、生态完整性、生计稳定性、治理能力等指标"""

TOOL_DESC_ENVFISH_FEEDBACK = """\
【EnvFish反馈回路摘要】
读取EnvFish工件中的反馈链和事件，分析环境-生态-社会的相互影响。
...
TOOL_DESC_ENVFISH_INTERVENTION = """\
【EnvFish干预比较】
读取EnvFish工件中的干预变量和执行结果，比较不同政策/干预的效果与摩擦。
...
ENV_PLAN_SYSTEM_PROMPT = """\
你是一个「生态社会影响推演报告」的撰写专家，擅长从EnvFish模拟工件中总结区域级污染扩散、生态反馈和政策干预后果。

【你的任务】
撰写一份面向决策者的EnvFish报告，回答：
1. 当前场景的基线与灾害/干预注入是什么？
2. 污染或风险是如何在区域之间扩散、传导和放大的？
3. 人类、生态、治理与基础设施之间产生了哪些反馈回路？
4. 哪些区域/角色最脆弱，为什么？
5. 干预前后差异如何，是否出现次生灾害？
6. 该模拟有哪些不确定性与模型边界？
...
【章节数量限制】
- 固定 6 个章节
- 章节必须与下面的结构一致，不要改写章节职责
- 可以根据结果调整标题措辞，但含义必须保持一致
...
请输出 JSON 格式：
{
  ...
  "summary": "一句话摘要",
  "sections": [
    {"title": "场景摘要与模型设定", "description": "..."},
    {"title": "区域扩散态势与传播链", "description": "..."},
    {"title": "人类-自然反馈回路", "description": "..."},
    {"title": "脆弱区域与关键角色", "description": "..."},
    {"title": "干预方案对比", "description": "..."},
    {"title": "不确定性与模型边界", "description": "..."}
  ]
}
"""

ENV_PLAN_USER_PROMPT_TEMPLATE = """\
【EnvFish 模拟场景】
模拟需求/注入变量：
...
- 关系数: {total_edges}
- 实体类型分布: {entity_types}
- 活跃实体数: {total_entities}

【可引用的工件事实】
{related_facts_json}

请严格围绕六个固定章节规划报告。优先突出区域扩散、人类-自然反馈、脆弱性、干预比较与不确定性。"""

ENV_SECTION_SYSTEM_PROMPT_TEMPLATE = """\
你是一个「EnvFish生态社会推演报告」的撰写专家，正在撰写其中一个章节。

报告标题: {report_title}
报告摘要: {report_summary}
模拟需求/注入变量: {simulation_requirement}
章节标题: {section_title}
...
- 区域扩散态势与传播链：说明污染/风险如何跨区域扩散、顺序、强度和滞后
- 人类-自然反馈回路：说明生态受体、人类角色、治理系统之间的反馈
...
【格式规范】
- 仍然使用 ReACT 模式
- 每次回复只能做一件事：要么调用一个工具，要么输出 Final Answer
- 章节正文不要使用 # / ## / ### 标题
- 调用工具时必须输出以下格式：
  <tool_call>
  {{"name": "工具名称", "parameters": {{"参数名": "参数值"}}}}
  </tool_call>
- 输出最终章节时必须以 "Final Answer:" 开头，且不要再包含工具调用
"""

ENV_SECTION_USER_PROMPT_TEMPLATE = """\
已完成章节内容（请避免重复）：
{previous_content}

【当前任务】撰写章节: {section_title}

请先思考需要什么信息，然后调用一个最相关的工具。输出最终章节时，请直接基于工件信息写作，重点突出区域扩散、人类-自然反馈与干预差异。"""

ENV_CHAT_SYSTEM_PROMPT_TEMPLATE = """\
你是一个简洁高效的EnvFish报告助手。
...
【规则】
1. 优先基于EnvFish工件和报告内容回答问题
2. 简洁、明确地回答，不要泛泛而谈
3. 仅在工件信息不足时才调用工具
4. 如果信息不确定，要明确说明是半定量推断
5. 调用工具时使用 <tool_call> 格式，最终回答以 Final Answer: 开头
...
class ReportAgent:
    ...
    def _define_tools(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "envfish_summary": {
                ...
                "description": TOOL_DESC_ENVFISH_SUMMARY,
                "parameters": {
                    "limit": "摘要条目数量（可选，默认8）"
                }
            },
            "envfish_spread_forecast": {
                "name": "envfish_spread_forecast",
                "description": TOOL_DESC_ENVFISH_SPREAD,
                "parameters": {
                    "limit": "摘要条目数量（可选，默认8）"
                }
            },
            "envfish_vulnerability_ranking": {
                "name": "envfish_vulnerability_ranking",
                "description": TOOL_DESC_ENVFISH_VULNERABILITY,
                "parameters": {
                    "limit": "摘要条目数量（可选，默认8）"
                }
            },
            "envfish_feedback_summary": {
                "name": "envfish_feedback_summary",
                "description": TOOL_DESC_ENVFISH_FEEDBACK,
                "parameters": {
                    "limit": "摘要条目数量（可选，默认8）"
                }
            },
            "envfish_intervention_comparison": {
                "name": "envfish_intervention_comparison",
                "description": TOOL_DESC_ENVFISH_INTERVENTION,
                "parameters": {
                    "limit": "摘要条目数量（可选，默认8）"
                }
            }
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict[str, Any], report_context: str = "") -> str:
        ...
        try:
            ...
            elif tool_name == "envfish_summary":
                limit = parameters.get("limit", 8)
                if isinstance(limit, str):
                    limit = int(limit)
                return self.zep_tools.get_envfish_summary(self.simulation_id, limit=limit)

            elif tool_name == "envfish_spread_forecast":
                limit = parameters.get("limit", 8)
                if isinstance(limit, str):
                    limit = int(limit)
                return self.zep_tools.get_envfish_regional_spread_summary(self.simulation_id, limit=limit)

            elif tool_name == "envfish_vulnerability_ranking":
                limit = parameters.get("limit", 8)
                if isinstance(limit, str):
                    limit = int(limit)
                return self.zep_tools.get_envfish_vulnerability_ranking(self.simulation_id, limit=limit)

            elif tool_name == "envfish_feedback_summary":
                limit = parameters.get("limit", 8)
                if isinstance(limit, str):
                    limit = int(limit)
                return self.zep_tools.get_envfish_feedback_summary(self.simulation_id, limit=limit)

            elif tool_name == "envfish_intervention_comparison":
                limit = parameters.get("limit", 8)
                if isinstance(limit, str):
                    limit = int(limit)
                return self.zep_tools.get_envfish_intervention_comparison(self.simulation_id, limit=limit)
    ...
    def _get_tools_description(self, include_envfish: Optional[bool] = None) -> str:
        """生成工具描述文本"""
        desc_parts = ["可用工具："]
        envfish_mode = self._is_envfish_context() if include_envfish is None else include_envfish
        tool_names = list(self.tools.keys())
        if not envfish_mode:
            tool_names = [
                name for name in tool_names
                if not name.startswith("envfish_")
            ]
    ...
    def _is_envfish_context(self, context: Optional[Dict[str, Any]] = None) -> bool:
        """判断当前模拟是否为 EnvFish。"""
        context = context or self._get_simulation_context()
        if not context:
            return False
        return bool(
            context.get("simulation_kind") == "envfish"
            or context.get("envfish_available")
            or context.get("envfish")
        )

    def _build_envfish_outline(self, context: Dict[str, Any], progress_callback: Optional[Callable] = None) -> ReportOutline:
        """构建 EnvFish 固定章节大纲。"""
        ...
        if progress_callback:
            progress_callback("planning", 35, "正在生成EnvFish固定报告结构...")

        system_prompt = """你是一个EnvFish报告标题与摘要设计专家。你的任务是基于模拟工件，生成简洁但准确的报告标题与摘要。

要求：
1. 只返回JSON
2. 标题要体现生态社会影响推演、区域扩散或沙盘推演
3. 摘要要概括场景模式、扩散模板、区域扩散和干预摩擦
4. 不要输出章节，只输出标题和摘要
返回格式：{"title": "...", "summary": "..."}"""
...
EnvFish上下文：
{json.dumps(envfish_data, ensure_ascii=False, indent=2)}
...
请生成报告标题和摘要。"""
        ...
        title = "EnvFish生态社会影响推演报告"
        summary = "基于半定量区域扩散与人类-自然反馈回路的EnvFish推演分析。"
        try:
            ...
            title = response.get("title", title) or title
            summary = response.get("summary", summary) or summary
        except Exception as e:
            logger.warning(f"EnvFish 大纲标题摘要生成失败，使用默认值: {e}")

        sections = [
            ReportSection(title="场景摘要与模型设定"),
            ReportSection(title="区域扩散态势与传播链"),
            ReportSection(title="人类-自然反馈回路"),
            ReportSection(title="脆弱区域与关键角色"),
            ReportSection(title="干预方案对比"),
            ReportSection(title="不确定性与模型边界"),
        ]

        if progress_callback:
            progress_callback("planning", 80, "EnvFish报告结构已确定")
    ...
    def plan_outline(
        ...
        if self._is_envfish_context(context):
            if progress_callback:
                progress_callback("planning", 25, "检测到EnvFish工件，生成固定报告结构...")
            outline = self._build_envfish_outline(context, progress_callback=progress_callback)
            if self.report_logger:
    ...
    def _generate_section_react(
        ...
        if envfish_mode:
            system_prompt = ENV_SECTION_SYSTEM_PROMPT_TEMPLATE.format(
    ...
    def chat(
        ...
        if envfish_mode:
            system_prompt = ENV_CHAT_SYSTEM_PROMPT_TEMPLATE.format(
                simulation_requirement=self.simulation_requirement,
                envfish_summary=context.get("envfish_summary", "（暂无EnvFish摘要）"),
...
class ReportManager:
    ...
    @classmethod
    def _resolve_report_folder(cls, report_id: str) -> str:
        ...
        if os.path.exists(meta_path):
            try:
                ...
                if str(data.get("artifact_mode") or "live") == "frozen":
                    artifact_root = str(data.get("artifact_root") or "").strip()
    ...
    def _get_report_markdown_path(cls, report_id: str) -> str:
        """获取完整报告Markdown文件路径"""
        return os.path.join(cls._resolve_report_folder(report_id), "full_report.md")
    
    @classmethod
    def _get_outline_path(cls, report_id: str) -> str:
        """获取大纲文件路径"""
        return os.path.join(cls._resolve_report_folder(report_id), "outline.json")
    
    @classmethod
    def _get_progress_path(cls, report_id: str) -> str:
        """获取进度文件路径"""
        return os.path.join(cls._resolve_report_folder(report_id), "progress.json")
    
    @classmethod
    def _get_section_path(cls, report_id: str, section_index: int) -> str:
        """获取章节Markdown文件路径"""
        return os.path.join(cls._resolve_report_folder(report_id), f"section_{section_index:02d}.md")
    
    @classmethod
    def _get_agent_log_path(cls, report_id: str) -> str:
        """获取 Agent 日志文件路径"""
        return os.path.join(cls._resolve_report_folder(report_id), "agent_log.jsonl")
    
    @classmethod
    def _get_console_log_path(cls, report_id: str) -> str:
        """获取控制台日志文件路径"""
        return os.path.join(cls._resolve_report_folder(report_id), "console_log.txt")
    ...
    def get_generated_sections(cls, report_id: str) -> List[Dict[str, Any]]:
        ...
        返回所有已保存的章节文件信息
        """
        folder = cls._resolve_report_folder(report_id)
    ...
    def save_report(cls, report: Report) -> None:
        ...
        # 保存大纲
        if report.outline and report.artifact_mode != "frozen":
            cls.save_outline(report.report_id, report.outline)
        
        # 保存完整Markdown报告
        if report.markdown_content and report.artifact_mode != "frozen":
