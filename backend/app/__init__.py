...
def create_app(config_class=Config):
    ...
    # 恢复上一次服务进程中断遗留的后台任务状态
    from .services.startup_recovery import recover_interrupted_tasks
    recovered_tasks = recover_interrupted_tasks()
    if should_log_startup and recovered_tasks:
        logger.warning("已标记 %s 个中断任务为失败", len(recovered_tasks))
    ...
    # 注册蓝图
    from .api import graph_bp, simulation_bp, report_bp, map_bp, scene_bp, control_bp, golden_cases_bp
