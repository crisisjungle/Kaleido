...
const normalizeMapProjection = (projection, graph) => {
  ...
  const projectSeedId = projectData.value?.map_seed_id
  const simSeedId = currentSimulation.value?.map_seed_id
  const isGeoAnchored = hasMapSeedContext || projectSeedId || simSeedId

  const distanceFromDesired = haversineKm(currentLat, currentLon, desiredCenter.lat, desiredCenter.lon)
  
  if (isGeoAnchored) {
    if (distanceFromDesired > 30 || (Math.abs(currentLat - 20) < 0.1 && Math.abs(currentLon) < 0.1)) {
      return { ...projection, center: desiredCenter }
    }
    return projection
  }

  if (distanceFromDesired < 30) return projection
...
const handleNextStep = (params = {}) => {
  ...
  if (params.simulationArchitecture) {
    addLog(`推演架构: ${params.simulationArchitecture}`)
  }
...
const loadSimulationData = async () => {
  try {
    ...
    if (initialCenter && !mapProjection.value) {
      mapProjection.value = { center: initialCenter }
    }
