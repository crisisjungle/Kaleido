<template>
  <div class="envfish-step envfish-step2">
    <div class="hero">
      <div class="hero-copy">
        <div class="eyebrow">KALEIDO / STEP 2</div>
        <h2>生态社会场景设计</h2>
        <p>
      ...
      <div class="hero-metrics">
        <div class="metric-card">
          <span class="metric-label">模拟</span>
          <span class="metric-value mono">{{ simulationId || '待生成' }}</span>
        </div>
        <div class="metric-card">
        ...
        <div class="metric-card">
          <span class="metric-label">主体</span>
    ...
    <section class="workspace-shell">
      <div class="workspace-topbar">
        <div class="workspace-copy">
          <div class="eyebrow workspace-eyebrow">场景工作台</div>
          <h3>推演参数、风险对象与场景结构</h3>
          <p>先校准推演参数和变量注入，再检查风险对象、区域划分、代理体配置和关系骨架。配置输入与结构预览分开，避免互相遮蔽。</p>
        </div>
...
async function handlePrepare(options = {}) {
  ...
  emit('update-status', 'processing')
  addLog(
    `${autoTriggered ? '自动' : '手动'}提交 Kaleido 场景配置: ${scenarioMode.value} / ${hazardTemplateMeta.value.label} / ${searchMode.value} / ${temporalProfileLabel.value}`
  )
